...
def G_paper(
    ...
    num_facies          = 3,            # Number of facies types.
    ...
    output_facies       = True,        # Decide whether the output is a facies model or several probability cubes for each facies.
    **kwargs):                          # Ignore unrecognized keyword args.
    ...
    def tofm_prob(x, res):    # obtain softmax of facies types (facies proportion channels) fm with shape of [N, channels, x_dim, y_dim, z_dim]
        ...
        with tf.variable_scope('tofm_prob_lod%d' % lod):           
            return tf.math.softmax(act(apply_bias(conv3d(x, fmaps=num_facies, kernel=1, gain=1, use_wscale=use_wscale))), axis = 1)          
    ...
    if structure == 'linear':
        ...
        for res in range(3, out_sizes_log2_lg - latent_sizes_log2_lg + 2 + 1):  # (out_sizes_log2_lg - latent_sizes_log2_lg + 2) is equavalent to 7 for 128x128x32, since the trained G may be used for enlarged fields (see 1280x1280x320)
            ...
            with tf.variable_scope('Grow_lod%d' % lod):
                ...
                ups_fac = upscale_factor(x, full_usc_thr)
        cubes_out = tofm(cubes_out, out_sizes_log2_lg) if output_facies else cubes_out  # final output facies model     
    ...
    if structure == 'recursive':
        def grow(x, prob, wellfc, res, lod):
            ...
            return cube()
        cubes_out_pre = grow(combo_in, probcubes_in, wellfacies_in, 2, out_sizes_log2_lg - latent_sizes_log2_lg + 2 - 2)  
        cubes_out = tofm(cubes_out_pre, np.log2(cubes_out_pre.shape[-1].value).astype(int)) if output_facies else cubes_out_pre
