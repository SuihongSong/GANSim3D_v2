...
def train_progressive_gan(
    ...
    misc.save_image_grid(grid_reals, os.path.join(result_subdir, 'reals.png'), drange=training_set.dynamic_range, grid_size=grid_size) #  drange was also changed into [0,1]
    grid_fakes = grid_fakecubes[:,:,:,:,8]      
    misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % 0), drange=training_set.dynamic_range, grid_size=grid_size)
    if cond_well:
        grid_wellfacies = grid_wellfaciescube_process[:,0:1,:,:,8] + grid_wellfaciescube_process[:,1:2,:,:,8]
        misc.save_image_grid(grid_wellfacies/2, os.path.join(result_subdir, 'wellfacies.png'), drange=training_set.dynamic_range, grid_size=grid_size)
    if cond_prob:
        grid_probs = grid_probcubes[:,:,:,:,8]
        misc.save_image_grid(grid_probs, os.path.join(result_subdir, 'probimages.png'), drange=training_set.dynamic_range, grid_size=grid_size)     
    ...
    while cur_nimg < total_kimg * 1000:
        ...
        if cur_nimg >= tick_start_nimg + sched.tick_kimg * 1000 or done:
            ...
            if cur_tick % cube_snapshot_ticks == 0 or done:
                grid_fakecubes = Gs.run(grid_latents, grid_labels_cube, grid_wellfaciescube_process, grid_probcubes, minibatch_size=sched.minibatch//config.num_gpus)
                grid_fakes = grid_fakecubes[:,:,:,:,8]
                misc.save_image_grid(grid_fakes, os.path.join(result_subdir, 'fakes%06d.png' % (cur_nimg // 1000)), drange=training_set.dynamic_range, grid_size=grid_size)
